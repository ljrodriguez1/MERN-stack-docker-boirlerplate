...
const eventHandlers = notificationCenter => {
  ...
  return {
    [ADD_USER]: ({ payload, ws, userConnectionID, cookie }) => {
      console.log('Hola amiwo estoy añadiendo un nuevo user pls as print en el login ;(')
      console.log(payload)
      if (cookie){
      const { name = '' } = payload;
      ...
      if (userAlredyExist(payload.id)) {
        console.log(cookie)
        updateReconnectedUser();

        return;
      }
      console.log('lamentablemente no encontre otro user con esa cokie', cookie)
    ...
    [REMOVE_USER]: ({ ws, cookie }) => {
      console.log('here i am ,' , cookie)
      if (cookie){
      console.log(cookie, 'yhis is my coockie')
      ...
      if (isUserStillConnectedFromAnotherTab) {
        console.log('toy conectao en otro lao')
        return;
      }
